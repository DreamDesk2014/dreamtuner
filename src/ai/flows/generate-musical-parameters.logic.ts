...
// This file contains ONLY the pure business logic.
// It does not use `defineFlow` and can be safely imported by other server-side files.
...
// Define the MasterMusicParameterContext schema for type safety
const MasterMusicParameterContextSchema = z.object({
  name: z.string(),
  preferredTempoRange: z.object({
    min: z.number(),
    max: z.number(),
  }).optional(),
  preferredKeyContext: z.string().optional(),
  preferredInstrumentHints: z.array(z.string()).optional(),
  preferredMelodicContour: z.string().optional(),
  preferredMelodicPhrasing: z.string().optional(),
  notesOnDynamics: z.string().optional(),
}).optional();

// Define the input schema for this logic function
export const GenerateMusicalParametersInputSchema = z.object({
  ...
  masterParameterContext: MasterMusicParameterContextSchema,
});

export type GenerateMusicalParametersInput = z.infer<typeof GenerateMusicalParametersInputSchema>;

// Define the output schema for this logic function
export const GenerateMusicalParametersOutputSchema = z.object({
  ...
  instrumentHints: z.array(z.string()).describe('Instrument suggestions for the music, including descriptive adjectives.'),
  rhythmicDensity: z.number().min(0).max(1).describe('A value between 0 and 1 representing the rhythmic density.'),
  harmonicComplexity: z.number().min(0).max(1).describe('A value between 0 and 1 representing the harmonic complexity.'),
  targetValence: z.number().min(-1).max(1).describe("A value between -1 and 1 representing the valence of the music."),
  targetArousal: z.number().min(-1).max(1).describe("A value between -1 and 1 representing the arousal of the music."),
  generatedIdea: z.string().describe('A structured description of the musical piece.'),
  melodicContour: z.string().optional().describe("The overall shape or direction of the primary melody."),
  melodicPhrasing: z.string().optional().describe("The characteristic phrasing of the main melody."),
  melodicEmphasis: z.string().optional().describe("How prominent the main melody is in the overall texture."),
});

export type GenerateMusicalParametersOutput = z.infer<typeof GenerateMusicalParametersOutputSchema>;

// The full, default prompt template
...
{{#if isKidsMode}}
  ...
  - melodicPhrasing: Suggest 'short_motifs' or 'legato' patterns.
...
// This is the core logic function, now safe to be imported anywhere.
export async function generateMusicalParametersLogic(
  ...
  try {
    ...
    // Fetch dynamic prompt from Firebase logic
    ...
    try {
        ...
        if (dynamicPrompt?.promptTemplate) {
            ...
            console.log(`Using dynamic prompt: "${fetchedPromptName}"`);
        } else {
            console.log("Using default built-in prompt template.");
        }
    } catch (error) {
        console.error("Failed to fetch AI prompt from Firestore. Falling back to default.", error);
    }
    
    // Fetch master parameter set from Firebase logic
    ...
    try {
        ...
        if (masterParamSet) {
            console.log(`Applying Master Parameter Set: "${masterParamSet.name}"`);
        ...
        console.error("Failed to fetch master parameter set. Continuing without it.", error);
    }
