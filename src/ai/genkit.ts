...
// This will be effective when server actions import this file.
dotenvConfig();

const apiKey = process.env.GEMINI_API_KEY || process.env.GOOGLE_API_KEY;

if (!apiKey && process.env.NODE_ENV !== 'production') {
  // In development, if the key isn't found after loading .env,
  // it's a clearer sign of a local setup issue.
  // In production, we assume the environment variable is set directly via Firebase console.
  console.warn(
    `[Genkit Setup Warning] GEMINI_API_KEY or GOOGLE_API_KEY not found in environment variables after attempting to load .env.
    If running locally, please ensure your API key is set in a .env file at the project root.
