// src/lib/toneService.ts

'use client'; // This MUST be the very first line of the file, before anything else.
...
export const generateWavFromMusicParameters = async (params: MusicParameters): Promise<Blob | null> => {
    ...
    let noteDurationNotation: string = "4n";
  ...
  if (drumEventsToSchedule.length > 0 || isKidsMode) {
  ...
  try {
    const audioBuffer = await Tone.Offline(async (offlineContext: Tone.OfflineContext) => {
      ...
      melodyNotesToSchedule.forEach((ev) => {
        ...
        if (isMelodySampler) {
          const samplerInstance = melodySynth as any;
      ...
      if (arpeggioSynth && arpInstrumentSetupResult) {
        arpeggioNotesToSchedule.forEach((ev) => {
            ...
            if (isArpSampler) {
              const samplerInstance = arpeggioSynth as any;
              let finalNoteToPlayForSampler = noteToPlay;
