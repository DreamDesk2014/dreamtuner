...
const getSynthConfigurations = (
  ...
  let configs: any = {
    melody: { oscillator: { type: 'fmsine' as const, harmonicity: 2.0, modulationIndex: 7, modulationType: "triangle" as const }, envelope: { attack: 0.02, decay: 0.4, sustain: 0.6, release: 0.8 }, volume: -6 },
    ...
    chords: { oscillator: { type: 'amtriangle' as const, harmonicity: 0.5, modulationType: "sine" as const }, volume: -18, envelope: { attack: 0.15, decay: 0.6, sustain: 0.4, release: 1.3 } },
    arpeggio: { oscillator: { type: SAFE_OSC_TYPE, partials: [1, 0.2, 0.05] }, envelope: { attack: 0.01, decay: 0.1, sustain: 0.1, release: 0.15 }, volume: -20 },
    kick: { pitchDecay: 0.04, octaves: 6, oscillator: { type: "sine" as const }, envelope: { attack: 0.001, decay: 0.3, sustain: 0.005, release: 1.0, attackCurve: "exponential" as const }, volume: -3 },
    snare: { noise: { type: 'pink' as const, playbackRate: 0.8 }, volume: -10, envelope: { attack: 0.001, decay: 0.08, sustain: 0, release: 0.12 } },
    hiHat: { frequency: 400, envelope: { attack: 0.001, decay: 0.03, release: 0.03 }, harmonicity: 3.5, modulationIndex: 15, resonance: 2500, octaves: 1.0, volume: -15 },
    piano: {
...
export const generateWavFromMusicParameters = async (params: MusicParameters): Promise<Blob | null> => {
  ...
  const totalMelodyDurationSeconds = measureDurationSeconds * 2; // Target two measures of melody
  let accumulatedMelodyDuration = 0;
  let scaleIndex = 0;

  while (accumulatedMelodyDuration < totalMelodyDurationSeconds && scaleNoteNames.length > 0) {
      const rhythmicDensity = params.rhythmicDensity || 0.5;
      let noteDurationNotation: string;
      let restChance = 0.1; // Base chance for a rest

      if (rhythmicDensity < 0.3) { // Sparse
          noteDurationNotation = Math.random() < 0.6 ? "4n" : "2n";
          restChance = 0.3;
      } else if (rhythmicDensity < 0.7) { // Moderate
          noteDurationNotation = Math.random() < 0.7 ? "8n" : "4n";
          restChance = 0.15;
      } else { // Dense
          noteDurationNotation = Math.random() < 0.7 ? "16n" : "8n";
          restChance = 0.05;
      }

      const noteDurationSec = Tone.Time(noteDurationNotation).toSeconds();

      if (Math.random() < restChance && accumulatedMelodyDuration > 0) { // Don't start with a rest
          console.log(`${logPrefix}_ALGO_MELODY] Adding rest for ${noteDurationNotation} at ${melodyCurrentTime.toFixed(3)}s`);
          melodyCurrentTime += noteDurationSec;
          accumulatedMelodyDuration += noteDurationSec;
      } else {
          const noteName = scaleNoteNames[scaleIndex % scaleNoteNames.length];
          // Simple contour: mostly stepwise, occasional small jump
          if (Math.random() < 0.2) { // 20% chance of a small jump
              scaleIndex += (Math.random() < 0.5 ? 2 : -2);
          } else {
              scaleIndex += (Math.random() < 0.6 ? 1 : -1); // Favor moving up slightly
          }
          scaleIndex = (scaleIndex + scaleNoteNames.length) % scaleNoteNames.length; // Ensure positive index

          melodyNotesToSchedule.push({ time: melodyCurrentTime, note: noteName, duration: noteDurationNotation, velocity: 0.7 });
          overallMaxTime = Math.max(overallMaxTime, melodyCurrentTime + noteDurationSec);
          melodyCurrentTime += noteDurationSec;
          accumulatedMelodyDuration += noteDurationSec;
      }
      if (melodyCurrentTime > totalMelodyDurationSeconds + startOffset) break; // Ensure we don't vastly overshoot
  }
  console.log(`${logPrefix}_ALGO_MELODY] Generated ${melodyNotesToSchedule.length} melody notes.`);
  ...
  const numDrumMeasures = Math.ceil(chordCurrentTime / measureDurationSeconds); // Use chordCurrentTime for drum loop length
